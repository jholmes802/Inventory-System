...
def transaction(part_num:str, qty:int, typ:str, dest:str = None)-> bool:
    _sql_conn = conn_gen()
    _sql_cur = cursor_gen(_sql_conn)
    if part_num_check(part_num):
        tstamp = now()
        sql_transaction = "INSERT INTO transactions ('part_number', 'type', 'qty', 'dest', 'datetime') VALUES ('" + part_num + "', '" + typ + "', " + qty + ",'" + dest + ",'" + tstamp + "');"
